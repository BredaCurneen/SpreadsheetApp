...
// Plain CII is a built-in format of @e-invoice-eu/core (FormatCIIService,
// registered under the key 'CII') — there is no separate @e-invoice-eu/cii
// package. This is the same real library already used for XRechnung and
// ZUGFeRD in this app, just with the plain EN16931 CII format selected
// instead of the XRechnung- or Factur-X-specific profiles.
const CII_FORMAT = 'CII';
...
export class CiiService {
  ...
  /**
   * Generates plain CII XML (EN16931, UN/CEFACT syntax) from the UBL XML the
   * app already produces, via @e-invoice-eu/core's real InvoiceService.
   */
  async generate(ublXml: string): Promise<string> {
    ...
    const options: InvoiceServiceOptions = { format: CII_FORMAT, lang: 'en' };
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'CII XML generation failed.';
      throw badRequest(`CII XML generation failed: ${message}`);
    }
  }
}

// ── UBL → InvoiceData (same reversal approach as facturx.service.ts /
//    xrechnung.service.ts / zugferd.service.ts, kept self-contained here). ───
...
function parseUblToInvoiceData(xml: string): InvoiceData {
  ...
  if (lines.length === 0) {
    throw badRequest('UBL invoice has no invoice lines — cannot generate CII XML.');
  }
...
// (Same mapping as xrechnung.service.ts / zugferd.service.ts, including the
// BR-S-02/BR-Z-02/BR-AE-02 fix: VAT numbers go into `cac:PartyTaxScheme`, not
// `cac:PartyLegalEntity/cbc:CompanyID`. Duplicated here to keep this service
// self-contained rather than importing from a sibling service file.)
