import { InvoiceData, TaxCategory, ValidationError } from '../types/invoice.types';
 ...
 */
const VALID_TAX_CATEGORIES = new Set<TaxCategory>(['S', 'Z', 'E', 'AE', 'K', 'O']);

/** Categories that must carry a 0 % VAT rate (BR-Z-1, BR-E-1, BR-AE-1, BR-K-1, BR-O-1). */
const ZERO_RATE_CATEGORIES = new Set<TaxCategory>(['Z', 'E', 'AE', 'K', 'O']);
...
export class ValidatorService {
  validate(invoice: InvoiceData): ValidationError[] {
    ...
    if (!invoice.issueDate) {
      ...
      errors.push(
        fatal('BR-03-FMT', `Issue date "${invoice.issueDate}" is not ISO 8601 (YYYY-MM-DD).`, '/Invoice/cbc:IssueDate'),
      );
    }
    if (invoice.dueDate && !ISO_DATE_RE.test(invoice.dueDate)) {
      errors.push(
        warn('BR-DUEDATE-FMT', `Due date "${invoice.dueDate}" is not ISO 8601 (YYYY-MM-DD).`, '/Invoice/cbc:DueDate'),
      );
    }
    if (!invoice.typeCode) {
    ...
    if (!invoice.currencyCode) {
      ...
      errors.push(
        fatal('BR-05-FMT', `Currency code "${invoice.currencyCode}" must be ISO 4217 (3 uppercase letters).`, '/Invoice/cbc:DocumentCurrencyCode'),
      );
    }
    ...
    if (!invoice.seller.countryCode) {
      ...
      errors.push(
        fatal('BR-09-FMT', `Seller country code "${invoice.seller.countryCode}" must be ISO 3166-1 alpha-2.`, '/Invoice/cac:AccountingSupplierParty'),
      );
    }
    if (!invoice.seller.vatNumber) {
      errors.push(
        warn('BR-14', 'Seller VAT identifier is missing. Required unless exempt.', '/Invoice/cac:AccountingSupplierParty'),
      );
    }
    ...
    if (!invoice.buyer.countryCode) {
      ...
      errors.push(
        fatal('BR-11-FMT', `Buyer country code "${invoice.buyer.countryCode}" must be ISO 3166-1 alpha-2.`, '/Invoice/cac:AccountingCustomerParty'),
      );
    }
    ...
    for (const line of invoice.lines) {
      ...
      if (line.taxCategory !== undefined) {
        if (!VALID_TAX_CATEGORIES.has(line.taxCategory)) {
          errors.push(
            fatal('BR-CAT', `Line ${line.lineNumber}: "${line.taxCategory}" is not a valid EN16931 tax category. Allowed: S, Z, E, AE, K, O.`, loc),
          );
        } else if (ZERO_RATE_CATEGORIES.has(line.taxCategory) && line.vatRate !== 0) {
          errors.push(
            fatal(`BR-${line.taxCategory}-1`, `Line ${line.lineNumber}: category "${line.taxCategory}" requires 0% VAT rate, got ${line.vatRate}%.`, loc),
          );
        }
      }
    }
    ...
    if (Math.abs(computedNet - invoice.netAmount) > 0.02) {
      errors.push(
        fatal('BR-CO-10', `Net amount mismatch: lines sum to ${computedNet}, header states ${invoice.netAmount}.`, '/Invoice/cac:LegalMonetaryTotal'),
      );
    }
    if (Math.abs(computedGross - invoice.grossAmount) > 0.02) {
      errors.push(
        fatal('BR-CO-15', `Gross amount mismatch: computed ${computedGross}, header states ${invoice.grossAmount}.`, '/Invoice/cac:LegalMonetaryTotal'),
      );
    }
  ...
  hasFatals(errors: ValidationError[]): boolean {
    return errors.some((e) => e.severity === 'fatal');
  }
}

function fatal(code: string, message: string, location?: string): ValidationError {
  return { code, severity: 'fatal', message, location };
}

function warn(code: string, message: string, location?: string): ValidationError {
