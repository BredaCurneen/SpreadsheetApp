import { SpreadsheetService, SheetMatrix } from './spreadsheet.service';
import {
  InvoiceData,
  InvoiceLineItem,
  InvoiceParty,
...
const VALID_TAX_CATS = new Set<string>(['S', 'Z', 'E', 'AE', 'K', 'O']);

export class MappingService {
  constructor(private readonly ss: SpreadsheetService) {}

  apply(matrix: SheetMatrix, cfg: MappingConfig): InvoiceData {
    const f = cfg.fields;
    const cell = (ref: CellRef | undefined): string =>
      ref ? this.ss.getCellValue(matrix, ref.row, ref.col) : '';
  ...
  private extractParty(matrix: SheetMatrix, mapping: PartyMapping): InvoiceParty {
    const cell = (ref: CellRef | undefined): string =>
      ref ? this.ss.getCellValue(matrix, ref.row, ref.col) : '';
  ...
  private extractLines(
    matrix: SheetMatrix,
    startRow: number,
    cols: MappingConfig['fields']['lineItemColumns'],
  ): InvoiceLineItem[] {
    ...
    for (let row = startRow; row < matrix.length; row++) {
      ...
      if (!description) break; // first blank description row marks end of contiguous line block
      ...
      const taxCategoryRaw = cols.taxCategory !== undefined
        ...
        : '';
      const taxCategory: TaxCategory | undefined = VALID_TAX_CATS.has(taxCategoryRaw)
      ...
      logger.debug(`Line ${lineNum}: "${description}" qty=${quantity} price=${unitPrice} vat=${vatRate}% cat=${taxCategory ?? 'derived'}`);
