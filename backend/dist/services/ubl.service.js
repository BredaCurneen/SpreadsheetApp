...
const PEPPOL_CUSTOMIZATION = 'urn:cen.eu:en16931:2017#compliant#urn:fdc:peppol.eu:2017:poacc:billing:3.0';
...
class UblService {
    generate(invoice) {
        ...
        const effectiveCategories = new Set(invoice.lines.map(l => l.taxCategory ?? (l.vatRate > 0 ? 'S' : 'Z')));
        const hasK = effectiveCategories.has('K');
        const inv = (0, xmlbuilder2_1.create)({ version: '1.0', encoding: 'UTF-8' }).ele('Invoice', {
            xmlns: NS.ubl,
            'xmlns:cac': NS.cac,
            'xmlns:cbc': NS.cbc,
        });
        ...
        if (invoice.dueDate)
            inv.ele('cbc:DueDate').txt(invoice.dueDate);
        inv.ele('cbc:InvoiceTypeCode').txt(invoice.typeCode);
        if (invoice.note)
            inv.ele('cbc:Note').txt(invoice.note);
        ...
        if (invoice.buyerReference)
            inv.ele('cbc:BuyerReference').txt(invoice.buyerReference);
        // ── InvoicePeriod (BG-14) ──────────────────────────────────────────────
        ...
        if (invoice.invoicePeriodStart || invoice.invoicePeriodEnd) {
            ...
            if (invoice.invoicePeriodStart)
                period.ele('cbc:StartDate').txt(invoice.invoicePeriodStart);
            if (invoice.invoicePeriodEnd)
                period.ele('cbc:EndDate').txt(invoice.invoicePeriodEnd);
        }
        ...
        sellerLegal.ele('cbc:RegistrationName').txt(invoice.seller.registrationName ?? invoice.seller.name);
        if (invoice.seller.vatNumber) {
            sellerLegal.ele('cbc:CompanyID').txt(invoice.seller.vatNumber);
        }
        // Seller Contact BG-6 — DE-R-002
        const hasContact = invoice.seller.contactName || invoice.seller.contactPhone || invoice.seller.contactEmail;
        if (hasContact) {
            const contact = sellerEl.ele('cac:Contact');
            if (invoice.seller.contactName)
                contact.ele('cbc:Name').txt(invoice.seller.contactName);
            if (invoice.seller.contactPhone)
                contact.ele('cbc:Telephone').txt(invoice.seller.contactPhone);
            if (invoice.seller.contactEmail)
                contact.ele('cbc:ElectronicMail').txt(invoice.seller.contactEmail);
        }
        // ── Buyer ──────────────────────────────────────────────────────────────
        ...
        buyerLegal.ele('cbc:RegistrationName').txt(invoice.buyer.registrationName ?? invoice.buyer.name);
        // ── Delivery ───────────────────────────────────────────────────────────
        ...
        if (needsDelivery) {
            ...
            if (hasK || deliveryCountry) {
                ...
                if (deliveryCity)
                    addr.ele('cbc:CityName').txt(deliveryCity);
                if (deliveryPostal)
                    addr.ele('cbc:PostalZone').txt(deliveryPostal);
 ...
 */
function resolveEndpoint(party) {
    ...
    if (party.vatNumber) {
        ...
        if (scheme)
            return { id: party.vatNumber, schemeId: scheme };
    }
    return null;
}
function round2(n) {
